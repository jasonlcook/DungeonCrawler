        ...
        public static Dungeon? Get(Guid dungeonId) {
            try {
                using (var context = new DungeonContext()) {
                    return context.Dungeons.Include("Adventurer").Include("Levels").Include("Levels.Tiles").Include("Levels.Tiles.Monsters").Include("Messages").Where(d => d.Id == dungeonId).FirstOrDefault();
                }
            } catch (Exception ex) {
